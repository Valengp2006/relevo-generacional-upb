...
function applyState(isSlideChange = false) {
  ...
  if (huellaCaptionEl) {
    ...
    if (slide.hasPhoto) {
      huellaCaptionEl.style.left = '43%';
      huellaCaptionEl.style.maxWidth = '28vw';
    } else {
  ...
  if (ctaContainer && qrCode) {
    ctaContainer.style.left = slide.hasPhoto ? '43%' : '50%';
  ...
  opts.centerX = slide.hasPhoto ? width * 0.43 : width * 0.5;
